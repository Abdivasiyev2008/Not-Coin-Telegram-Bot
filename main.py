...
os.environ.setdefault('DJANGO_SETTINGS_MODULE',
                      'config.settings')  # 'config.settings' you must enter your settings file
django.setup()

from botapp.models import User, \
    RefFriendModel  # Import your models
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_message = """
Hey! Welcome to RPG T-Coin Bot!
Tap on the coin and see your balance rise.

RPG T-Coin Bot is a cutting-edge financial platform where users can earn tokens by leveraging the mining app's various features. The majority of R-P-G Token (TAPS) distribution will occur among the players here.
...
A huge reward awaits you at the end of the project for inviting your friends. It remains a mystery for now\n\n"""
    ...
    if referred_user_id:
        ...
        if not user_exists:
            ...
            if created:
                ...
                # Bot gives 1000 coins to referral of owner
                await update_user_coins(referred_user_id, 1000)
            else:
                # If bot finds user on Database, it doesn't do anything
    ...
    await update.message.reply_text(
        ...
        #     ] if is_subscribed else [],  # User can see PLAY BUTTON when subscribe the channel
        #     resize_keyboard=True
        # ),
    )
